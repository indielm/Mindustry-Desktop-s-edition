...
public class Player extends Unit implements BuilderTrait, CarryTrait, ShooterTrait{
    ...
    void freecamAI(){
        float xd = Core.camera.position.x - x, yd = Core.camera.position.y - y;
        boolean movePlayer = false;
        if(placeQueue.size > 0)
        {
            BuildRequest currentBuild = placeQueue.first();
            xd = currentBuild.x * tilesize - x;
            yd = currentBuild.y * tilesize - y;
            if((xd * xd + yd * yd) > 4096) movePlayer = true;
            if((xd * xd + yd * yd) < 1024){
                xd *= -1;
                yd *= -1;
                movePlayer = true;
            }
        } else if(((xd * xd + yd * yd) > 64) || getFloorOn().isLiquid){
            xd = Core.camera.position.x - x;
            yd = Core.camera.position.y - y;
            movePlayer = true;
        }
        if(getFloorOn().isLiquid) isBoosting = true;
        if (mining!=null) movePlayer = false;
        if(movePlayer){
            movement.x += xd;
            movement.y += yd;
            movement.nor();
            movement.x *= mech.boostSpeed;
            movement.y *= mech.boostSpeed;
            isBoosting = true;
    ...
    protected void updateMech(){
        ...
        if(!freecam) isBoosting = Inputs.keyDown("dash") && !mech.flying;
        if(Inputs.keyTap("freeCam")) freecam = !freecam;
        ...
        float xa = 0, ya = 0;
        if(!freecam){
            xa = Inputs.getAxis(section, "move_x");
            ya = Inputs.getAxis(section, "move_y");
        }else freecamAI();
