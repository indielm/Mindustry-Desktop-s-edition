...
public class PlacementFragment extends Fragment{
    ...
    /*    if (Inputs.keyRelease(Input.CONTROL_LEFT)) prevTileX = -1; // Mouse drag conv
        ...
        if ((Inputs.keyDown(Input.CONTROL_LEFT)) && (Inputs.keyDown(Input.MOUSE_LEFT))
            ...
            int destX = world.toTile(Graphics.mouseWorld().x), destY = world.toTile(Graphics.mouseWorld().y);
            if (prevTileX == -1) {
                prevTileX = destX;
                prevTileY = destY;
            }
            int tileX = prevTileX, tileY = prevTileY;           //start at last dragged tile
            while (!((tileX == destX) && (tileY == destY))) {   // while tileX,tileY isn't at mouse's tile
                int dx = tileX - destX, dy = tileY - destY;
                if (abs(dx) > abs(dy)) {                        // move horz or vert by 1 tile based on which difference is bigger
                    input.rotation = (int)signum(dx) + 1;
                    tileX -= signum(dx);
                } else {
                    input.rotation = (int)signum(dy) + 2;
                    tileY -= signum(dy);
                }
                if (input.rotation != prevRot) {               // if rotation changed, redo the last conv  to new rotation
                    if (players[0].getPlaceQueue().size > 0)
                        players[0].getPlaceQueue().removeLast();  // bugfix, still glitchy, PlaceQueue has issues with placing rotated convs on queued tiles
                    //else input.tryBreakBlock(prevTileX,prevTileY);
                    input.tryPlaceBlock(prevTileX, prevTileY);
                }
                prevRot = input.rotation;
                prevTileX = tileX;
                prevTileY = tileY;
